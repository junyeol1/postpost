...
function CarpoolInfo() {
  ...
  useEffect(()=>{        
      ...
      fetch(`http://localhost:5000/api/carpoolboard?id=${boardId}`)
      .then(res=>{
          return res.json();
      })
      .then(data=>{
      ...
      fetch(`http://localhost:5000/api/carpoolpassenger?boardId=${boardId}`)
      .then(res=>{
          return res.json();
      })
      .then(data=>{
  ...
  const delBoard = () =>{
    console.log(boardId);
      ...
      passengerList.forEach(passenger=>{
          fetch(`http://localhost:5000/api/carpoolpassenger${boardId+passenger.userId}`, {method:"DELETE"})
          .then(res=>{
              if(res.ok){
                  console.log("동승 신청 삭제 함");
                  // 게시글 삭제
                  // fetch(`http://localhost:3001/board/${boardId}`, {method:"DELETE"})
                  fetch(`http://localhost:5000/api/carpoolboard/${boardId}`, {method:"DELETE"})
                  .then(res=>{
                      if(res.ok){
                          alert("게시글이 삭제되었습니다.")
    ...
    <CarpoolInfoWrapper>
      ...
      <ContentWrapper>
        ...
        <StyledButton width={"100%"} height={"30px"} onClick={addPassenger} disabled={
          ...
          {currentData.driver === "driver" ? "동승하기" : "픽업하기"}
        </StyledButton> :
        null
        }
      </ContentWrapper>
